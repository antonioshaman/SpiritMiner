...
from utils.formatting import format_entry_strategy

router = Router()


class CalcEntryStates(StatesGroup):
    waiting_for_input = State()


@router.callback_query(MenuAction.filter(F.action == "calc_entry"))
async def cb_calc_entry(callback: CallbackQuery, state: FSMContext) -> None:
    await callback.message.edit_text(
        "\U0001f4c8 <b>Рассчитать вход</b>\n\n"
        "Введите тикер или название монеты:",
        reply_markup=back_to_menu_kb(),
        parse_mode="HTML",
    )
    await state.set_state(CalcEntryStates.waiting_for_input)
...
async def handle_entry_input(message: Message, state: FSMContext) -> None:
    ...
    if len(coins) == 1:
        ...
        text = format_entry_strategy(coin, score)
    ...
    await message.answer(
        ...
        reply_markup=coin_list_kb(coins, 0, "entry"),
        parse_mode="HTML",
    )


@router.callback_query(CoinAction.filter(F.action == "entry"))
async def cb_coin_entry(callback: CallbackQuery, callback_data: CoinAction) -> None:
